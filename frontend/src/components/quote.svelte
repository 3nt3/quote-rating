...
{#if !compact}
  <div
    class="bg-slate-800 rounded-xl p-6 flex-1 sm:w-80 flex flex-col gap-4 shadow-lg border-2 border-slate-700 text-slate-300"
  >
    ...
    <!-- content -->
    {#if quote.image_url}
      <img class="rounded-md" src={quote.image_url} alt="no alt tag" />
    {/if}
    <SvelteMarkdown
      source={quoteContent}
